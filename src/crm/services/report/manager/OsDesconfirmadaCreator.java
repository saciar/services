...
public class OsDesconfirmadaCreator implements ReportService {
	private static final String REPORT_OS_NAME = "jasper/osDescofirmada.jasper";
	...
	private static OsDesconfirmadaCreator instance;
	private Date fechaEmision;
		
	private static final Log log = LogFactory.getLog(OsDesconfirmadaReport.class);
	
	public static synchronized OsDesconfirmadaCreator instance() {

			if (instance == null) {
				instance = new OsDesconfirmadaCreator();
			}
	...
	public JasperPrint createOSReport(long nroPpto) throws RemoteException, JRException {
		...
		OrdenServiciosDesconfirmada[] eventos = OsDesconfirmadaReport.instance().findByNroPpto(nroPpto);
	...
	private String getEstadoOS (long nroPpto){
		try{
			...
			Date fechaOS = OSDesconfirmadaReportDSBuilder.getDatefromString(fechaOSSt);
			if (PresupuestosManager.instance().isPptoCancelado(nroPpto)){
				...
				Date fechaCanc = OSDesconfirmadaReportDSBuilder.getDatefromString(fechaCancSt);
	...
	private Date getMaximaFechaActualizacion(long nroPpto, int estado) throws RemoteException{
		...
		return OSDesconfirmadaReportDSBuilder.getDatefromString(fecha);
	}
	
	/**
	 * Crea un reporte de eventos
	 * 
	 * @param eventos el listado de eventos que se iterar�
	 * @param subTitle el titulo que recibir� el reporte, puede ser week o day
	 * @param year el a�o del reporte
	 * @return
	 * @throws RemoteException
	 * @throws JRException
	 */
	@SuppressWarnings("unchecked")
	private JasperPrint createReport(OrdenServiciosDesconfirmada[] eventos, String title, String subTitle) throws RemoteException, JRException {
		...
		JasperPrint jasperPrint = JasperFillManager.fillReport(oSReport, parameters, OSDesconfirmadaReportDSBuilder.toJRMap(eventos));
 ...
 * @author hernux
 */
class OSDesconfirmadaReportDSBuilder {
	...
	@SuppressWarnings("unchecked")
	public static JRMapArrayDataSource toJRMap(OrdenServiciosDesconfirmada[] ordenes){
		Object[] data = new Object[ordenes.length];
		
		for (int i=0;i<ordenes.length;i++) {
			OrdenServiciosDesconfirmada ordenServicio = ordenes[i];
			Map map = new HashMap();
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static JRMapArrayDataSource toJRMap(OrdenServiciosDesconfirmadaSala[] salas, int numeroSala){
		Object[] data = new Object[salas.length];
		
		for (int i=0;i<salas.length;i++) {
			OrdenServiciosDesconfirmadaSala sala = salas[i];
			Map map = new HashMap();
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static JRMapArrayDataSource toJRMap(OrdenServiciosDesconfirmadaServicios[] servicios){
		Object[] data = new Object[servicios.length];
		hasProveeedores = false;
		for (int i=0;i<servicios.length;i++) {
			OrdenServiciosDesconfirmadaServicios servicio = servicios[i];
			Map map = new HashMap();
