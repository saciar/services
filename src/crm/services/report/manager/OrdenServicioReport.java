...
public class OrdenServicioReport implements OrdenServicioReportSEI,ReportService {
	
	private static final long SYS_SETTINGS_EMAIL_COLD = 2;
	private static final long SYS_SETTINGS_EMAIL_COLD_CHILE = 5;
	private static final long SYS_SETTINGS_EMAIL_COLD_MENDOZA = 6;
	...
	private static final Log log = LogFactory.getLog(OrdenServicioReport.class);
	
	private static String estado;
	
	public static synchronized OrdenServicioReport instance() {
	...
	public OrdenServicio[] findByNroPpto(long nroPpto) throws RemoteException{
		...
		OrdenServicio[] results = new OrdenServicio[list.size()];
		for (int i=0; i< results.length;i++){ 
			results[i] = new OrdenServicio();
	...
	private OrdenServicioSala[] findSalasByPresupuesto(Session session,long codigoSala, long nroPpto) {
		//"SELECT * FROM VW_RPT_WEEK where fechaEvento >= '2006-04-23' and fechaEvento <= '2006-04-25'";
		...
		OrdenServicioSala[] results = new OrdenServicioSala[list.size()];
		for (int i=0; i< results.length;i++){ 
			results[i] = new OrdenServicioSala();
			Object[] row = (Object[]) list.get(i);
	...
	private OrdenServicioServicio[] findServiciosBySala(Session session,long num_sala, long nroPpto) {
		//"SELECT * FROM VW_RPT_WEEK where fechaEvento >= '2006-04-23' and fechaEvento <= '2006-04-25'";
		...
		OrdenServicioServicio[] results = new OrdenServicioServicio[list.size()];
		for (int i=0; i< results.length;i++){ 
			results[i] = new OrdenServicioServicio();
			Object[] row = (Object[]) list.get(i);
	...
	private EventoProveedor[] getProveedoresByNroPpto(Session session, long nroPpto){
		
		List list = session.createSQLQuery
			(
			"select p.pr_nombreproveedor as nombre, p.pr_telefono1 as tel1, " +
			"p.pr_telefono2 as tel2, p.pr_calle as calle, p.pr_nro as numero, p.pr_piso as piso, " +
			"p.pr_dpto as depto " +
			"from TX_GASTOS_CONTRATACIONES contrat " +
			"inner join MST_PROVEEDORES p on p.pr_codproveedor = contrat.gc_codproveedor " +
			"where contrat.gc_ppto_nroppto = ? "
			)
			.addScalar("nombre", Hibernate.STRING)
			.addScalar("tel1", Hibernate.STRING)
			.addScalar("tel2", Hibernate.STRING)
		...
		EventoProveedor[] results = new EventoProveedor[list.size()];
		for (int i=0; i< results.length;i++){ 
			results[i] = new EventoProveedor();
			Object[] row = (Object[]) list.get(i);
			
			results[i].setProveedor((String)row[0]);
			results[i].setTelefonos((String)row[1] + " / " + (String)row[2]);
			results[i].setDomicilio((String)row[3] + " " + (String)row[4] + " piso: " + (String)row[5] + " depto: " + (String)row[6]);
	...
	public boolean sendOSByEmail2(long nroPpto, String fechaInicial, String fechaFinal, String evento, String usuarioId, String codLugar, String emailDestino) throws RemoteException{
		try {
			...
			if(lugarMail!= null && !lugarMail.equals("") && !lugarMail.equals("OS@congressrental.com")){	
				mailMessage.setToAddress(new String[]{emailDestino,lugarMail, "jgodoy@congressrental.com.ar"},new String[]{"Cold",lugarNombre,"jgodoy"});
			}
			else
				mailMessage.setToAddress(new String[]{emailDestino,"jgodoy@congressrental.com.ar"},new String[]{"Cold", "jgodoy"});
			...
			mailMessage.setSubject("Orden de servicio Nro"+nroPpto+" "+estado);
			mailMessage.setFilePath(PDF_LOCATION + "ordenServ_" + nroPpto + ".pdf");
			mailMessage.setFileName("OrdenServicio_" + nroPpto + ".pdf");			
			mailMessage.setHtmlBody(getHTMLBody(nroPpto, fechaInicial, fechaFinal, evento, lugarNombre));
			
			mailMessageGerencia.setSubject("Orden de servicio Nro"+nroPpto+" "+estado);
			mailMessageGerencia.setFilePath(PDF_LOCATION + "ordenServ_" + nroPpto + ".pdf");
			mailMessageGerencia.setFileName("OrdenServicio_" + nroPpto + ".pdf");			
	...
	public boolean sendOSByEmail(long nroPpto, String fechaInicial, String fechaFinal, String evento, String usuarioId, String codLugar) throws RemoteException{
		try {
			...
			if(lugarMail!= null && !lugarMail.equals("") && !lugarMail.equals("OS@congressrental.com")){	
				mailMessage.setToAddress(new String[]{"saciar@congressrental.com"},new String[]{"Sergio"});
			}
			else
				mailMessage.setToAddress(new String[]{"saciar@congressrental.com"},new String[]{"sergio"});			
			//mailMessage.setBccAddress(new String[]{ReportService.EMAIL_CRM_CONTROL}, new String []{"Cold"});
			...
			mailMessage.setFilePath(PDF_LOCATION + "ordenServ_" + nroPpto + ".pdf");
			mailMessage.setFileName("OrdenServicio_" + nroPpto + ".pdf");
			mailMessage.setFilePath(PDF_LOCATION + "Informe_" + nroPpto + ".pdf");
			mailMessage.setFileName("Informe_" + nroPpto + ".pdf");
			mailMessage.setHtmlBody(getHTMLBody(nroPpto, fechaInicial, fechaFinal, evento, lugarNombre));
	...
	private String getHTMLBody(long nroPpto, String fechaInicial, String fechaFinal, String evento, String lugar){
		...
		html += "<body>";
		html += "<h1>";		
		html += "<FONT SIZE=2>ORDEN DE SERVICIO NRO: " + nroPpto + ".</FONT><BR><BR>";
		if(e != null)
	...
	public void savePdfFile(long nroPpto) throws RemoteException{		
		ReportBuilder.exportOSToPdf(nroPpto, PDF_LOCATION + "ordenServ_" + nroPpto + ".pdf");		
